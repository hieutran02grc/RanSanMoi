...
public class ConRan {
	...
	public void update() {
		...
		for(int i=1;i<doDai;i++) {
			if(x[0]==x[i]&&y[0]==y[i]) {
				...
				RanSanMoiJava.users.add(new User(name, String.valueOf(GameScreen.point)));
				GameScreen.isPlaying=false;
				GameScreen.isGameOver=true;	
				
				//reset point and level when game over
				GameScreen.point = 0;
				GameScreen.CurrentLevel = 1;
	...
	public void veRan(Graphics g) {
		...
		//draw snake's head
		if(vector==ConRan.GO_UP) g.drawImage(Data.HeadGoUp.getCurrentImage(), x[0]*20-7 +GameScreen.padding, y[0]*20-6 + GameScreen.padding, null);
		else if(vector==ConRan.GO_DOWN) g.drawImage(Data.HeadGoDown.getCurrentImage(), x[0]*20-7 +GameScreen.padding, y[0]*20-6 +GameScreen.padding, null);
		else if(vector==ConRan.GO_LEFT) g.drawImage(Data.HeadGoLeft.getCurrentImage(), x[0]*20-7 +GameScreen.padding, y[0]*20-6 +GameScreen.padding, null);
		else if(vector==ConRan.GO_RIGHT) g.drawImage(Data.HeadGoRight.getCurrentImage(), x[0]*20-7 +GameScreen.padding, y[0]*20-6 +GameScreen.padding, null);
	} 
}
